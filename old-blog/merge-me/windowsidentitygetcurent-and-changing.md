...
param([string[]]$servers)$servers | %{ $base = [microsoft.win32.registrykey]::OpenRemoteBaseKey('LocalMachine',$_) $sub = $base.OpenSubKey('System\CurrentControlSet\Control\Lsa',$true) $sub.SetValue('LsaLookupCacheMaxSize',0) Start-Sleep -Seconds 5 $sub.DeleteValue('LsaLookupCacheMaxSize') $sub.Close() $base.Close()}
~~~~
