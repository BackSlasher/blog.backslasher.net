...
This means I can't rely on the mappings for my scripts, meaning I can't easily differentiate 
...
While not an issue by itself, Amazon's metadata is unaware of this change, causing the metadata to disagree with the real data.

### The road I didn't take
The first solution that comes to mind is to modify the parameter mentioned in the entry (`sda_is_xvda`) to cause the kernel to start naming the devices in the "right" order.  
...
It's not that complicated (`echo sdb | perl -p -e 'substr($_,2,1)=~tr{a-j}{e-p}'` prints `sdf`), but I needed a way to make sure the server has this translation method enabled.  
