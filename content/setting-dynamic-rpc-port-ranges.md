...
first!  
### RP-Wut?
For those not familiar with it, RPC ([Remote Procedure
...
- Windows 2003: 1025-5000
- Windows 2008+: 49152-65535
- Windows + Exchange 2010: 6005-59530 
...
-   `Ports` (`REG_MULTI_SZ`): A list of ports Windows is allowed to
...
-   `PortsInternetAvailable` (`REG_SZ`): Set to `Y` (case
    insensitive) to make Windows use the `Ports` list as a whitelist (only
    use those ports), and `N` to use "Ports" as a blacklist (use all
    other ports). I see no sense in setting this to `N`, but...
-   `UseInternetPorts` (`REG_SZ`): Set to `Y`. I don't realize what
