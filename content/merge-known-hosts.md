...
for file in args.files:
  with open(file) as f:
    for line in f:
      ...
      hosts = [ host for host in hosts if host not in familiar_hosts ] # Filter out familiar hosts
      if not hosts: continue # skip this row if no hosts left
      familiar_hosts.update(set(hosts))
      output.write('%s %s\n' %
          (','.join(hosts), ' '.join(line_split))
          )
