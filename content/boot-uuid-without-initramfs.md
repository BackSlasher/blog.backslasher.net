...
Igor, assuming there's only one hard drive, always targets `/dev/sda1` as the hard drive. This was an issue for me, as I two hard drives - an internal one that I installed the OS on, and an external one that I connect every now and then.

The uEnv file (the entry point for CubieTruck's boot sequence) contains something like `rootfs=/dev/sda1`, and when both my drives where connected, my external hard drive was discovered first, meaning `/dev/sda1` was pointing to it, causing the kernel to panic as it couldn't find a proper root fs.  
...
```text
waiting for root device <Whatever I wrote in uEnv>
```
